...
if __name__ == "__main__":
    ...
    parser.add_argument("--method-dirs", nargs="+", type=str, 
                        default=["baseline", "real-guidance-0.5-cap", 
                                 "textual-inversion-1.0-0.75-0.5-0.25"])
    
    parser.add_argument("--method-names", nargs="+", type=str, 
                        default=["Baseline", "Real Guidance (He et al., 2022)", 
                                 "DA-Fusion (Ours)"])
    ...
    for dataset, logdir in zip(args.datasets, args.logdirs):

        for bdir, bname in zip(args.method_dirs, args.method_names):

            bpath = os.path.join(logdir, bdir)
    ...
    for i, dataset in enumerate(args.datasets):
        ...
        train_dataset = DATASETS[dataset.lower()](split="train")

        results = combined_dataframe[combined_dataframe["dataset"] == dataset]

        results["total_unobserved_examples"] = \
            results["class_name"].apply(lambda x: len(
                train_dataset.class_to_images[x.lower()]))
        ...
        for j, examples_per_class in enumerate(
            ...
            legend = fig.legend(handles, args.method_names,
        ...
        for j, examples_per_class in enumerate(
            ...
            axis = sns.lineplot(x="total_unobserved_examples", y="value", hue="method", 
                                data=results2, errorbar=('ci', 68),
                                linewidth=4, palette=color_palette,
                                ax=axs)
            ...
            axis.set_xlabel("Total Unobserved Examples Per Class", 
                            fontsize=24, fontweight='bold', labelpad=12)

            axis.set_title(f"Dataset = {dataset} (Observed Examples Per Class = {examples_per_class})",
                           fontsize=24, fontweight='bold', pad=12)
            ...
            plt.savefig(f"{args.name}-{dataset}-{examples_per_class}-lineplot.pdf")
            plt.savefig(f"{args.name}-{dataset}-{examples_per_class}-lineplot.png")